...
def aov_plot(x, y, y_err, res_dict, target, field, color_by_time=True, save=False):

    per = res_dict['periods']
    freq = 1/per 
    power = res_dict['lspvals']

    best_fap = 1-np.min(periodbase.saov.analytic_false_alarm_probability(res_dict, x))
    ...
    ax2.set_ylabel('AoV $\Theta$', fontsize=14)
    ...
    if save:
        output_dir = f'/data/tierras/fields/{field}/sources/plots/aov_periodograms'
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
            set_tierras_permissions(output_dir)
        plt.savefig(output_dir+f'/{best_fap:.2f}_{target}_aov_periodogram.png', dpi=200)
        set_tierras_permissions(output_dir+f'/{best_fap:.2f}_{target}_aov_periodogram.png')
...
def main(raw_args=None):
    ...
    for i in range(n_lcs):
        ...
        x -= x[0]
        res_dict = periodbase.saov.aov_periodfind(x, y, y_err, magsarefluxes=True)
        aov_plot(x, y, y_err, res_dict, target, field, save=True)
