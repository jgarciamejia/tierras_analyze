...
for ref_as_target in complist:
	masked_complist = copy.copy(complist)
	mask= ~np.isin(masked_complist,ref_as_target) 
	masked_complist = masked_complist[mask]
	print (ref_as_target,mask,complist,masked_complist)

	# Load raw target and reference fluxes into global lists
	full_bjd, bjd_save, full_flux, full_err, full_reg, full_flux_div_expt, full_err_div_expt = ld.make_global_lists_refastarget(ref_as_target,lcpath,target,ffname,exclude_dates,masked_complist,ap_radius=ap_radius)
